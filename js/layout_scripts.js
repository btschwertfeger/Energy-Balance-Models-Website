...
tebm_resetbtn.onclick = function () {
  ...
  tebm_sliders.forEach((element, index) => {
    ...
    document.getElementById(tebm_variabels[index] + "_sliderAmount").innerHTML =
      default_value;
  });
...
for (let entry = 0; entry < tebm_sliders.length; entry++) {
  tebm_sliders[entry].oninput = function () {
    document.getElementById(tebm_sliders[entry].id + "rAmount").innerHTML =
      document.getElementById(tebm_sliders[entry].id).value;
  };
